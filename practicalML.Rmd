...
plot(confusion_DT$table,col=confusion_DT$byClass, 
     main = paste("Decision Tree Accuracy = ",round(confusion_RF$overall['Accuracy'], 4)))
```
...
Based on the metrics presented above, the Random Forest (RF) performed the best in the prediction task. A total of 500 trees were used by the RF with an accuracy of 99.46% in the validation set. Five fold cross validation is performed in the model building in order to get a more accurate measurement of the performance of the trained model. This model is applied in the holdout dataset and the first 20 predictions is presented above. 
